...
##
## File:
##    plot_station_beam.py
##
## Description:
##    Plot the IE613 station beam using a DFT on the visibility coverage.
...
##
## Examples:
##   python2.7 plot_ILOFAR_beam.py /Users/eoincarley/LOFAR/data/2017.08.21/AntennaField.conf
##
## Testing:
##
## Author:
...
time_0 = calendar.timegm( time.strptime("2017-06-21 12:00:00", "%Y-%m-%d %H:%M:%S") )
...
while time_0 < time_1:
    ...
    img=img.real
    img=numpy.log(img)
    plt.imshow(img, extent = (-1.0*pixels, pixels, pixels, -1.0*pixels), vmin = 0.51, vmax= 23.5)
    plt.xlabel('E-W')
    plt.ylabel('N-S')
    ...
    #pdb.set_trace()
    plt.show( )
    plt.savefig('./plotstationbeam'+str(format(image_num, '03'))+'.png')   # save the figure to file
    plt.close(fig)
    image_num+=1
