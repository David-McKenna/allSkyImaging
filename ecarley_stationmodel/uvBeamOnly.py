...
#	im = ax1.imshow(img, extent = (-1.0*pixels, pixels, pixels, -1.0*pixels), vmin = 0.51, vmax= 10.5)
	...
	if colIdx == subplotTuple[1] - 1:
		plt.show()
		plt.savefig(outputFolder + '/uv_beam_'+str(format(idx, '03'))+'.png')   # save the figure to file 
		plt.close(fig)
...
def mainCall(opts, args):
	...
	if not isinstance(hbaActivation, type(None)):
	...
	if outputFolder is None:
		outputFolder = 'IE613_uv_coverage'
		if pltLBA:
	...
	uvwLBA, freqLBA, __, __, xyzLBA, obsLBA, sunLBAObj, sourceObjLBA = antProc(antFieldFile, antArrFile, hbaRcuMode, obsTime, [0, hbaDeltas])
	uvwHBA, freqHBA, refTime, endTime, xyzHBA, obsHBA, sunHBAObj, sourceObjHBA = antProc(antFieldFile, antArrFile, hbaRcuMode, obsTime, [3, hbaDeltas])
	obs = obsLBA
	sunObj = sunLBAObj
	...
	while refTime <= endTime:
		...
		colIdx = 0
		if pltLBA:
			uvwLBA = processUVCoverage(idx, sunLBAObj, obs, xyzLBA, sourceObjLBA, obsLBA, freqLBA, colIdx, subplotTuple, 'HBA')
			colIdx += 1
		if pltHBA:
		...
		if not (pltLBA or pltHBA):
			fig.savefig(outputFolder + '/uv_beam_'+str(format(idx, '03'))+'.png')   # save the figure to file
			plt.close(idx)
	...
	if enableMp:
		...
		if pltLBA:
			for idx, asyncCallback in lbaRet:
				...
				beamPlot(idx, lbaImg, dummy, pixels, freqLBA, outputFolder, fftType, colIdx, subplotTuple, 'HBA')
	...
	subprocess.call([ffmpegLoc, '-y',  '-r',  '20',  '-i',  '{0}/uv_beam_%03d.png'.format(outputFolder), '-vb', '50M', "{0}/{0}.mpg".format(outputFolder)])
