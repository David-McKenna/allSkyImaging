...
##
## Description:
##    Plot the IE613 station UV coverage and the resulting station beam using 
##    a DFT on the visibility coverage.
##    This DOES NOT take into account antenna response. It therefore only
##    represents an 'ideal' station beam, in which the antennas have perfect 
##    (sidelobe-less) response.
##
## Disclaimer:
##
## Notes:
##
## Examples:
##   plot_UV_and_beam.py /Users/eoincarley/LOFAR/data/2017.08.21/AntennaField.conf
##
## Testing:
##
## Author:
...
time_0 = calendar.timegm( time.strptime("2017-06-21 07:00:00", "%Y-%m-%d %H:%M:%S") )
time_1 = calendar.timegm( time.strptime("2017-06-21 20:00:00", "%Y-%m-%d %H:%M:%S") )
image_num = 0
plt.ion()

#while time_0 < time_1:
time_utc = time.strftime("%Y/%m/%d %H:%M", time.gmtime(time_0)) #UTC (Use for Winter)
time_ist = time.strftime("%Y/%m/%d %H:%M", time.gmtime(time_0 + 60.0*60.0)) # Irish Standard Time (Summer)
obs.date = time_utc
sun = ephem.Sun()
sun.compute(obs)
src=sun
...
plt.xlabel('u ($\lambda$)')
plt.ylabel('v ($\lambda$)')
plt.title('UV-coverage at %s MHz for IE613 LBAs. Source: Sun @ %s IST' % (round(freq[0]/1e6, 1), time_ist))
...
plt.title('IE613 50 MHz LBA station beam. Source: Sun @ %s IST' % (time_ist))
plt.xlabel('E-W')
plt.ylabel('N-S')
...
pdb.set_trace()
plt.show( )
plt.savefig('./plotuvbeam'+str(format(image_num, '03'))+'.png')   # save the figure to file
image_num+=1
