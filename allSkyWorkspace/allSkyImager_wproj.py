...
def corrToSkyImage(correlationMatrix, posX, posY, posZ, obsFreq, lVec, mVec):
	# corrMat: (96,96,nChan)
	...
	wx = np.exp(-1j * k * posX * lVec) # (l, 96).T
	wy = np.exp(-1j * k * posY * mVec) # (m, 96).T
	wz = np.exp(-1j * k * posZ[:, :, np.newaxis] * np.sqrt(1 - np.abs(np.square(lVec) + np.square(mVec.T)))).transpose((1,2,0)) # (l, m, 96)

	print(wx.shape, wy.shape, wz.shape, wx[:, np.newaxis, :].shape, wy[:, :, np.newaxis].shape, np.multiply(wx[:, np.newaxis, :], wy[:, :, np.newaxis]).shape)
	weight = np.multiply(wz, np.multiply(wx[:, np.newaxis, :], wy[:, :, np.newaxis]).transpose((1,2,0)))[..., np.newaxis] # (l,m,96,1)
...
def generatePlots(inputCorrelations, antPos, plotOptions, dateArr, rcuMode, subband, multiprocessing = True, stationRotation = -11.9, plotX = True, plotY = True, mask = True, lVec = None, mVec = None, calibrationX = None, calibrationY = None, baselineLimits = None):
	...
	if plotX:
		...
		allSkyImX, __, __ = __processAllSkyIm(inputCorrelationsX, posX, posY, posZ, frequency, lVec, mVec, stationRotation, mask, plotOptions, labelOptionsX)
	...
	if plotY:
		...
		allSkyImY, __, __ = __processAllSkyIm(inputCorrelationsY, posX, posY, posZ, frequency, lVec, mVec, stationRotation, mask, plotOptions, labelOptionsY)
...
def __processAllSkyIm(inputCorrelations, posX, posY, posZ, frequency, lVec, mVec, stationRotation, mask, plotOptions, labelOptions):
	allSkyIm = corrToSkyImage(inputCorrelations, posX, posY, posZ, frequency, lVec, mVec)
	allSkyIm = np.rot90(allSkyIm, 3)
	allSkyIm = np.flipud(allSkyIm)

	# Scipy cannot handle nans apparently. Remove them and then replace all 100s (constant fill value) with nans afterwards.
	cacheNans = np.isnan(allSkyIm)
	allSkyIm[cacheNans] = 100.
	allSkyIm = scipy.ndimage.rotate(allSkyIm, stationRotation, mode = 'constant', cval = 100., reshape = False)
	allSkyIm[allSkyIm == 100.] = np.nan

	# This shouldn't be needed, but for some reason one of the above statements is failing.
