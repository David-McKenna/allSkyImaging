...
basepath='/data/home/user1/data/'`date +"%Y"`'/'`date +"%m"`'/'`date +"%d"`
datapath=$basepath'/mode'$1'_00/'$4'/sb'$2'/'
...
if [ $rcumode -gt 4 ]; then
	echo 'Activating HBA tiles by the Effelsberg scheme.'
	python set_HBA_single_element_pattern.py -v --element_list Effelsberg > $datapath/HBA_elements.log 2>&1 
fi
rspctl --bitmode=$bits
