...
if test $# != 3; then
	echo "Incorrect inputs: ./dmck_allsky_backend_prebaked.sh <output folder> <minutes> <duty cycle>"
	exit;
...
while true; do
	# Call our generic observations script: <RCUMODE> <START SUBBAND> <END SUBBAND> <SKIP N SUBBANDS> <INT TIME (s)> <OUTPUT FOLDER SUFFIX> ('skip' if skip applying activation)
        rspctl --mode=3
        bash ./dmck_generic_subband_scan.sh 3 151 452 10 2 4 $folder_name'3/'


        rspctl --mode=5
        bash ./dmck_generic_subband_scan.sh 5 51 152 5 2 4 $folder_name'5/'
        bash ./dmck_generic_subband_scan.sh 5 150 401 50 2 4 $folder_name'5/' # SM - removed skip as mentioned in email with DMcK


        rspctl --mode=7
        bash ./dmck_generic_subband_scan.sh 7 51 152 10 2 4 $folder_name'7/'
